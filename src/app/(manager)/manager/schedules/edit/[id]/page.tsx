...
import {
  useGetWeeklyScheduleTemplateById,
...
export default function EditWeeklyScheduleTemplatePage() {
  const router = useRouter();
  const params = useParams();
  const id = params?.id ? Number(params.id) : 0;

  const { data: template, isLoading: isTemplateLoading } =
    useGetWeeklyScheduleTemplateById(id);
  ...
  useEffect(() => {
    if (template) {
      ...
      // Convert template items to include taskTemplate info
      const items: TemplateItemWithTask[] = template.templateItems.map(
        (item, idx) => ({
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              Không tìm thấy mẫu lịch
            </h1>
            <p className="text-muted-foreground">
              Mẫu lịch mà bạn đang tìm không tồn tại
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 max-w-full overflow-hidden">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Cập nhật thông tin mẫu lịch làm việc
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cập nhật tên, mô tả và công việc cho từng ngày
          </CardDescription>
        </CardHeader>
