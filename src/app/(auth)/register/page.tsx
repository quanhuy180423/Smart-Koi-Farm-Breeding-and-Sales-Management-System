...
import { RegisterRequest } from "@/lib/api/services/fetchAuth";
...
export default function SignUpPage() {
  ...
  useEffect(() => {
    if (registerSuccess && registeredEmail) {
      // Show verification dialog after successful registration
      setShowVerifyDialog(true);
    }
  }, [registerSuccess, registeredEmail]);

  const handleVerifySuccess = () => {
    // Reset register state and redirect to login
    resetRegisterState();
  ...
  return (
    <>
      <VerifyEmailDialog
        ...
        onOpenChange={(open) => {
          if (!open) {
            // Allow closing dialog only if not verifying
            resetRegisterState();
          }
          ...
          <div className="flex items-center justify-center p-4 lg:p-8 relative overflow-hidden bg-background">
            ...
            <Card className="w-full max-w-md mx-auto bg-card/95 backdrop-blur-md border-border/60 shadow-2xl relative z-10 transition-all duration-500 hover:shadow-primary/20 hover:shadow-2xl hover:-translate-y-1 rounded-2xl overflow-hidden">
              ...
              <CardContent className="pt-1 relative z-10">
                ...
                {/* Verify Email Button */}
                <Link href="/verify-email" className="block w-full mb-3">
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full border-2 border-accent text-accent hover:bg-accent hover:text-accent-foreground font-semibold transition-all duration-300 h-10 hover:shadow-lg hover:shadow-accent/20"
                  >
                    Xác thực lại email
                  </Button>
                </Link>
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground/70 mt-2 leading-relaxed">
                    Bằng cách đăng ký, bạn đồng ý với điều khoản sử dụng của
                    chúng tôi
                  </p>
                </div>
              </CardContent>
