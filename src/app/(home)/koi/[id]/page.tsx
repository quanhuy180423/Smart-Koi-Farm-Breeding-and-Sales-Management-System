...
const PedigreeNode = ({
  ...
  return (
    <div className="border border-indigo-300 rounded-lg p-3 text-center shadow-lg bg-white min-w-40 transform transition-all hover:scale-[1.02] hover:shadow-xl relative z-10">
      ...
      <p
        className={`text-xs font-bold ${koi?.gender === Gender.MALE ? "text-blue-600" : "text-pink-600"
          }`}
      >
                ...
                <CarouselContent className="-ml-2">
                  {koi.images.map((image, index) => (
                    <CarouselItem key={index} className="pl-2 basis-1/3">
                      <button
                        onClick={() => setSelectedImage(index)}
                        className={`aspect-square rounded-lg overflow-hidden border-2 transition-colors w-full ${selectedImage === index
                          ? "border-primary"
                          : "border-transparent"
                          }`}
                      >
