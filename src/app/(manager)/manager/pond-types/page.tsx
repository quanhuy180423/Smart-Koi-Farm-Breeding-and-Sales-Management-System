...
export default function PondTypeManagement() {
  ...
  const handleAddPondType = () => {
    if (!newPondType.type || !newPondType.recommendedQuantity) {
      return;
    }

    // Tự động generate tên loại hồ từ loại hồ và sức chứa
    const pondTypeLabel = getPondTypeLabel(
      newPondType.type as PondTypeEnum,
    ).label;
    const autoGeneratedName = `${pondTypeLabel} - ${newPondType.recommendedQuantity}`;

    const payload: PondTypeRequest = {
      typeName: autoGeneratedName,
                          ...
                          <div className="flex items-center justify-center">
                            <DropdownMenu>
                              ...
                              >
                                ...
                                >
                                  <Droplets className="mr-2 h-4 w-4 hover:text-white" />
                                  Thông số nước
                                </DropdownMenuItem>
                                ...
                                >
                                  {deletePondTypeMutation.isPending ? (
                                    ...
                                    <Trash2 className="mr-2 h-4 w-4 text-red-600 hover:text-white" />
                                  )}
                                  Xóa
                                </DropdownMenuItem>
                              </DropdownMenuContent>
