...
interface AddWeeklyScheduleModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: WeeklyScheduleTemplateRequest) => void;
  isPending: boolean;
}
...
export default function AddWeeklyScheduleModal({
  ...
  const handleSubmit = () => {
    ...
    // Convert template items to request format (remove taskTemplate and tempId properties)
    const payload = {
      name,
      description,
            ...
            <div className="border-t pt-4 space-y-4">
              ...
              {templateItems.length > 0 && (
                <div className="border rounded-lg overflow-hidden bg-white">
                  <div
                    className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-2 p-3"
