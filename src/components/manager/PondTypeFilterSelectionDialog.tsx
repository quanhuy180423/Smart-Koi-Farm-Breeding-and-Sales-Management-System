...
interface PondTypeFilterSelectionDialogProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  onSelect: (pondTypeId: number | null) => void;
  selectedId: number | null;
}

const PondTypeFilterSelectionDialog = ({
  isOpen,
  onOpenChange,
  onSelect,
  selectedId,
}: PondTypeFilterSelectionDialogProps) => {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="min-w-6xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Chọn loại hồ để lọc danh sách hồ cá
          </DialogDescription>
        </DialogHeader>
                ...
                <TableBody>
                  {pondTypes.length === 0 ? (
                    ...
                    pondTypes.map((type) => (
                      <TableRow
                        key={type.id}
                        onClick={() => setTempSelectedId(type.id)}
                        className={
                          type.id === tempSelectedId
                      ...
                      >
                        <TableCell>
                          <input
                            type="radio"
                            checked={type.id === tempSelectedId}
                            onChange={() => setTempSelectedId(type.id)}
...
export default PondTypeFilterSelectionDialog;
